                ...
                using (var scope = _serviceProvider.CreateScope())
                {
                    ...
                    var expiringSubs = await context.UserPackages
                        ...
                        .Where(s => s.Status == "Active" && 
                                    !s.ExpirationWarningSent && s.EndDate.Date <= threshold && s.EndDate.Date >= today)         
                    ...
                    foreach (var sub in expiringSubs)
                    {
                        ...
                        try 
                        {
                            await emailService.SendEmailAsync(sub.User.Email, "Twoja subskrypcja wkrótce wygasa",
                                body);
                            await notificationService.CreateNotificationAsync(sub.UserId, "Subskrypcja wygasa", 
                                $"Pakiet {sub.Package.Name} wygasa za 7 dni."
                                );
