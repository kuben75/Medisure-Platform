...
public class SubscriptionService : ISubscriptionService
{
    ...
    public async Task<(bool Success, string Message, object? Data)> SubscribeAsync(int packageId, SubscribeDto dto, string userId, string userEmail)
    {
        ...
        else
        {
            if (await _context.Users.AnyAsync(u => u.Pesel == dto.Pesel && u.Id != userId))
                return (false, "PESEL jest już zajęty.", null);
        ...
        var basePrice = _pricingService.CalculateBasePriceWithRiskFactor(package.PriceValue, package.Category, user.BirthDate);
    ...
    private async Task HandlePostPurchaseActions(ApplicationUser user, Package package, UserPackage sub, SubscribeDto dto, string priceString, DateTime start, DateTime end)
    {
        ...
        try
        {
            ...
            _ = _notificationService.NotifyAllAdminsAsync("Nowa sprzedaż", $"Użytkownik {user.Email} kupił {package.Name}.", "Sales");
        }
        catch (Exception ex)
        {
            await _logService.LogAsync("EMAIL_BLAD", $"Błąd po zakupie: {ex.Message}", "System", null, "Error");
        }
    }
    ...
    public async Task<(bool Success, string Message)> CancelSubscriptionAsync(int subscriptionId, string userId, string userName)
    {
        ...
        if (subscription == null) return (false, "Nie znaleziono subskrypcji.");
        if (subscription.Status == "Cancelled") return (false, "Już anulowana.");
        if (subscription.EndDate < DateTime.UtcNow) return (false, "Subskrypcja wygasła.");

        subscription.Status = "Cancelled";
        await _context.SaveChangesAsync();

        await _logService.LogAsync("ANULOWANIE", $"Anulowano {subscription.Package.Name}", userName, userId, "Info");
        await _notificationService.CreateNotificationAsync(userId, "Subskrypcja anulowana", $"Anulowano odnawianie pakietu {subscription.Package.Name}.", "System");

        return (true, "Subskrypcja anulowana.");
    }
}