...
export default function NavbarDrawers({
    ...
    return (
        <>
            ...
            <div
                className={`fixed top-0 right-0 h-screen w-[85vw] md:w-96 bg-white z-[70] transform transition-transform duration-300 ease-in-out shadow-2xl flex flex-col ${isNotificationsOpen ? "translate-x-0" : "translate-x-full"}`}>
                ...
                <div className="p-5 flex justify-between items-center border-b border-gray-100 bg-gray-50">
                    <div className="flex items-center gap-2">
                        ...
                        {unreadCount > 0 && <span
                            className="bg-red-500 text-white text-xs px-2 py-0.5 rounded-full font-bold">{unreadCount}</span>}
                    </div>
                    <div className="flex items-center gap-2">
                        {canClear && (
                        ...
                        <button onClick={() => setIsNotificationsOpen(false)}
                                className="p-2 bg-white rounded-full text-gray-500 hover:text-gray-800 transition-colors shadow-sm hover:shadow-md">
                ...
                {unreadCount > 0 && markAllAsRead && (
                    <div className="px-5 py-2 bg-blue-50/50 border-b border-blue-100 flex justify-end">
                        <button onClick={() => markAllAsRead()}
                                className="text-xs font-bold text-[#4E61F6] hover:text-blue-700 transition-colors">Oznacz
                ...
                <div className="flex-grow overflow-y-auto custom-scrollbar p-0">
                    {visibleNotifications && visibleNotifications.length > 0 ? (
                            ...
                            {visibleNotifications.map((notif: any) => (
                                <div key={notif.id} onClick={() => {
                                    ...
                                    <div className="flex justify-between items-start mb-1.5">
                                        ...
                                        <span
                                            className="text-[10px] text-gray-400 whitespace-nowrap ml-2">{new Date(notif.createdAt).toLocaleDateString()}</span>
                                    </div>
                        ...
                        <div
                            className="flex flex-col items-center justify-center h-full text-gray-400 p-8 text-center space-y-4">
                ...
                <div className="p-4 border-t border-gray-100 bg-gray-50 text-center">
                    <Link to={isAdmin ? "/admin?tab=notifications" : "/profile?tab=notifications"}
                          onClick={() => setIsNotificationsOpen(false)}
            ...
            <div
                className={`fixed top-0 right-0 h-screen w-[85vw] max-w-sm bg-white z-[70] transform transition-transform duration-300 ease-in-out lg:hidden shadow-2xl flex flex-col ${isMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
                    ...
                    {NAV_LINKS.map((link) => (
                        <li key={link.label}>
                            <Link to={link.to} onClick={() => setIsMenuOpen(false)}
                                  className={`block px-4 py-3.5 rounded-xl mb-1 font-medium transition-all ${location.pathname === link.to ? "bg-blue-50 text-[#4E61F6]" : "text-gray-600 hover:bg-gray-50"}`}>
                                {link.label}
                            </Link>
                        </li>
                    ))}
                    {isAdmin && (
                        <li>
                            <Link to="/admin" onClick={() => setIsMenuOpen(false)}
                                  className="flex items-center gap-3 px-4 py-3.5 rounded-xl mb-1 font-bold text-gray-700 bg-gray-50 hover:bg-gray-100 mt-4 border border-gray-100">
                    ...
                    {user ? (
                        <div className="space-y-4">
                            <div className="flex items-center gap-3">
                                <div
                                    className="w-12 h-12 bg-[#4E61F6] rounded-full flex items-center justify-center text-white text-xl font-bold">{user.firstName.charAt(0).toUpperCase()}</div>
                                <div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <Link to="/profile" onClick={() => setIsMenuOpen(false)}><Button variant="primary"
                                                                                                 className="w-full !py-2.5 !text-sm">Profil</Button></Link>
                                <Button variant="secondary"
                                        className="w-full !py-2.5 !text-sm bg-red-100 text-red-600 hover:bg-red-200"
                                        onClick={handleLogout}>Wyloguj</Button>
                            </div>
                        </div>
                    ) : (
                        <div className="space-y-3">
                            <Link to="/login" onClick={() => setIsMenuOpen(false)}><Button variant="outline"
                                                                                           className="w-full !py-3">Zaloguj
                                się</Button></Link>
                            <Link to="/rejestracja" onClick={() => setIsMenuOpen(false)}><Button variant="primary"
                                                                                                 className="w-full !py-3">Utwórz
                                konto</Button></Link>
                        </div>
                    )}
                </div>
