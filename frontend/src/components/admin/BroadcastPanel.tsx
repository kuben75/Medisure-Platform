...
export default function BroadcastPanel() {
    ...
    const getTypeStyles = (t: string) => {
        switch (t) {
            case 'Alert':
                return {bg: 'bg-red-50', border: 'border-red-200', iconColor: 'text-red-500', icon: <AlertIcon/>};
            case 'Success':
                return {
                    bg: 'bg-green-50',
            ...
            default:
                return {bg: 'bg-blue-50', border: 'border-blue-200', iconColor: 'text-blue-500', icon: <InfoIcon/>};
        }
    };
    ...
    return (
        <div
            className="mt-4 md:mt-8 bg-white p-6 md:p-8 rounded-2xl shadow-lg border border-gray-200 flex flex-col lg:flex-row gap-10">
            ...
            <div
                className="lg:w-1/3 bg-gray-50 rounded-2xl p-6 border border-gray-200 flex flex-col justify-center mt-6 lg:mt-0">
                <p className="text-xs font-bold text-gray-400 uppercase mb-4 text-center tracking-widest">Podgląd
                    odbiorcy</p>

                <div
                    className={`bg-white p-4 rounded-xl border-l-4 shadow-sm ${previewStyle.border} relative overflow-hidden transition-all duration-300`}>
                    <div className="flex gap-3">
                        <div
                            className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${previewStyle.bg} ${previewStyle.iconColor}`}>
                            {previewStyle.icon}
                        </div>
                ...
                <div className="mt-8 text-center hidden md:block">
                    ...
                    <p className="text-[10px] text-gray-400 mt-1">Powiadomienie pojawi się w panelu oraz przy
                        dzwonku.</p>
                </div>
            </div>
        </div>
