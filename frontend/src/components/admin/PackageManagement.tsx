            ...
            <div className="mt-4 md:mt-8 bg-white md:bg-slate-50 p-4 md:p-6 rounded-xl md:border border-gray-200">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                        ...
                        <Button variant="primary" className="!py-2.5 !px-4 text-sm shadow-md whitespace-nowrap justify-center" onClick={actions.handleOpenAdd}>
                            + Nowy pakiet
                        </Button>
                    </div>
                </div>
                ...
                <div className="md:hidden space-y-4">
                    {filteredPackages.map((pkg) => (
                        <div key={pkg.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm relative overflow-hidden">
                            <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${pkg.isFeatured ? 'bg-yellow-400' : 'bg-[#4E61F6]'}`}></div>
                            <div className="flex justify-between items-start mb-3 pl-3">
                                <div>
                                    <h3 className="font-bold text-gray-900 text-lg">{pkg.name}</h3>
                                    <div className="flex items-center gap-2 mt-1">
                                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide ${getCategoryBadgeStyle(pkg.category)}`}>
                                            {pkg.category}
                                        </span>
                                        {pkg.isFeatured && <span className="text-[10px] font-bold text-yellow-600 bg-yellow-50 px-2 py-0.5 rounded border border-yellow-100">★ HERO</span>}
                                    </div>
                                </div>
                            ...
                            <div className="flex justify-between items-center pl-3 pt-3 border-t border-gray-100">
                                ...
                                <div className="flex gap-2">
                                    <button onClick={() => actions.handleOpenEdit(pkg)} className="flex items-center gap-1 px-3 py-1.5 bg-indigo-50 text-indigo-700 text-xs font-bold rounded-lg border border-indigo-100 active:scale-95">
                                        <EditIcon className="w-4 h-4" /> Edytuj
                                    </button>
                                    <button onClick={() => actions.deletePackage(pkg.id)} className="flex items-center gap-1 px-3 py-1.5 bg-red-50 text-red-700 text-xs font-bold rounded-lg border border-red-100 active:scale-95">
