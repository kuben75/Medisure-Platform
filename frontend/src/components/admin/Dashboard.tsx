...
import type {IDashboardStats} from "../../types/types.ts";
import {StatCard} from "../ui/StatCard.tsx";
const API_URL_STATS = "https://localhost:44333/api/admin/stats";
    ...
    return (
        <div className="mt-8 bg-slate-50 p-6 rounded-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                />
                <StatCard
                    title="Pakiety w ofercie"
                    value={stats.totalPackagesAvailable}
                    icon={<PackageIconDashboard />}
                    colorClass="bg-green-50 text-green-600"
                />
                <StatCard
                    title="Aktywne subskrypcje"
                    value={stats.activeSubscriptions}
                    icon={<ActiveIconDashboard />}
                    colorClass="bg-indigo-50 text-indigo-600"
                />
                <StatCard
                    title="Subskrypcje wygasające (7 dni)"
                    value={stats.expiringSubscriptions}
                    icon={<WarningIconDashboard />}
                    colorClass="bg-yellow-50 text-yellow-600"
                />
            </div>
            <div className="mt-8">
                <h3 className="text-xl font-semibold text-gray-700 mb-4">Logi Systemowe</h3>
                <div className="bg-white p-4 rounded-lg shadow h-64 overflow-y-auto">
                    <p className="text-sm text-gray-500 font-mono">
                        [LOG] Logi serwerowe<br/>
                    </p>
                </div>
            </div>
        </div>
