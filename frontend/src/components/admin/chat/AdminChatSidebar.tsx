...
export const AdminChatSidebar = ({
    ...
    return (
        <div
            className={`w-full md:w-[320px] border-r border-gray-200 bg-white flex-col flex-shrink-0 ${selectedEmail ? 'hidden md:flex' : 'flex'}`}>
            <div className="p-4 border-b border-gray-100 bg-gray-50/50 space-y-3">
                ...
                <div className="relative">
                    ...
                    />
                    <svg className="w-4 h-4 text-gray-400 absolute left-3 top-3" fill="none" viewBox="0 0 24 24"
                         stroke="currentColor">
                ...
                <div className="flex gap-1 p-1 bg-gray-100 rounded-lg overflow-x-auto">
                    {[{id: 'all', label: 'Wszyscy'}, {id: 'unread', label: 'Nieprzeczytane'}, {
                        id: 'online',
                        label: 'Online'
                    }].map(tab => (
                        <button
                            ...
                            >
                                ...
                                <div className="overflow-hidden flex-1 flex flex-col justify-center">
                                    <div className="flex justify-between items-start mb-0.5">
                                        <span
                                            className={`text-sm truncate mr-2 ${hasUnread ? 'font-bold text-gray-900' : 'font-semibold text-gray-600'}`}>
                                            {c.displayName}
                                        </span>
                                        <span
                                            className={`text-[10px] whitespace-nowrap ${hasUnread ? 'text-[#4E61F6] font-bold' : 'text-gray-400'}`}>
                                    ...
                                    <div className="flex justify-between items-center mt-1">
                                        <p className={`text-xs truncate max-w-[150px] ${hasUnread ? 'text-gray-800 font-medium' : 'text-gray-500'}`}>
                                            ...
                                            {c.lastMsg.user === 'Admin' &&
                                                <span className="opacity-60 mr-1">Ty: </span>}
                                            {c.lastMsg.message}
                                        </p>
                                        {hasUnread && <span
                                            className="bg-[#4E61F6] text-white text-[10px] font-bold h-5 min-w-[20px] px-1 flex items-center justify-center rounded-full shadow-sm">{c.unread}</span>}
                                    </div>
                                </div>
                            </div>
                        );
