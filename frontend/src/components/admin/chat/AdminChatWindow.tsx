...
export const AdminChatWindow = ({conversation, onSend, onBack}: IAdminChatWindowProps) => {
    ...
    if (!conversation) {
        return (
            <div
                className="hidden md:flex w-full md:w-2/3 flex-col items-center justify-center h-full text-gray-400 bg-gray-50/50 p-4 text-center">
                <div
                    className="w-20 h-20 md:w-24 md:h-24 bg-white rounded-full shadow-sm flex items-center justify-center mb-6 border border-gray-100">
                    <svg className="w-8 h-8 md:w-10 md:h-10 text-gray-300" fill="none" viewBox="0 0 24 24"
                         stroke="currentColor">
    ...
    return (
        <div className="flex w-full md:w-2/3 flex-col bg-white relative">
            <div
                className="px-4 md:px-6 py-3 md:py-4 border-b border-gray-100 flex justify-between items-center bg-white shadow-[0_2px_10px_-10px_rgba(0,0,0,0.1)] z-10">
                    ...
                    <div>
                        <h3 className="text-sm font-bold text-gray-800 flex flex-col md:flex-row md:items-center md:gap-2">
                            ...
                            {conversation.email !== conversation.displayName && (
                                <span
                                    className="text-xs text-gray-400 font-normal md:before:content-['•'] md:before:mr-2 md:before:text-gray-300">
                                    {conversation.email}
                                </span>
                            )}
                        </h3>
                        <div className="flex items-center gap-1.5 mt-0.5">
                            <div
                                className={`w-1.5 h-1.5 rounded-full ${conversation.isOnline ? 'bg-green-500' : 'bg-gray-300'}`}></div>
            ...
            <div className="border-t border-gray-100 bg-white relative">
                <div
                    className={`absolute bottom-full left-0 w-full bg-slate-50 border-t border-gray-200 p-3 transition-all duration-300 ease-in-out shadow-inner ${showCanned ? 'translate-y-0 opacity-100 visible' : 'translate-y-4 opacity-0 invisible pointer-events-none'}`}>
                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider mb-2 ml-1">Szybkie
                        odpowiedzi</p>
                ...
                <form onSubmit={handleSend}
                      className="p-3 md:p-4 flex gap-2 md:gap-3 items-center z-20 relative bg-white">
