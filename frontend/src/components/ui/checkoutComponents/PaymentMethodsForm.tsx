...
export default function PaymentMethodsForm({
                                               paymentMethod, selectedBank, cardData,
    ...
    return (
        <div className="animate-fade-in space-y-6 md:space-y-8 pb-6 lg:pb-0">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                    {[
                        ...
                        <button key={m.id} onClick={() => {
                            ...
                            {paymentMethod === m.id && <div
                                className="absolute top-2 right-2 w-2 h-2 rounded-full bg-blue-500 animate-pulse"></div>}
                            <div
                                className={`transition-all duration-300 transform scale-90 md:scale-100 ${paymentMethod === m.id ? 'scale-100 md:scale-110' : 'grayscale opacity-70 group-hover:grayscale-0 group-hover:opacity-100'}`}>
                                {m.icon}
                            </div>
                            <span
                                className={`text-xs font-bold ${paymentMethod === m.id ? 'text-blue-700' : 'text-slate-500'}`}>{m.label}</span>
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div
                className="bg-white border border-gray-100 rounded-2xl p-5 md:p-8 shadow-sm relative overflow-hidden min-h-[280px]">
                {paymentMethod === 'card' && (
                    <div className="space-y-6 animate-fade-in relative z-10">
                        <div className="relative group">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 block">Numer
                                karty</label>
                ...
                {paymentMethod === 'transfer' && (
                    <div className="animate-fade-in h-full flex flex-col">
                        {!selectedBank ? (
                            <>
                                <h4 className="text-sm font-bold text-gray-700 mb-4 text-center">Wybierz swój bank lub
                                    BLIK</h4>
                                ...
                                <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 gap-3">
                                    ...
                                    {[
                                        ...
                                        <button key={bank.id}
                                                onClick={() => onSelectedBankChange(bank.id as TBankOptionType)}
                            ...
                            <div className="flex flex-col h-full animate-fade-in-up">
                                ...
                                {selectedBank === 'blik' ? (
                                    <div className="flex flex-col items-center justify-center py-4 flex-grow">
                                        <BlikLogo/>
                                        <label className="text-sm font-bold text-slate-500 mt-6 mb-4 block text-center">Wpisz
                                            kod z aplikacji bankowej</label>
                                        <div className="relative w-full flex flex-col items-center">
                                            ...
                                            {errors.blik &&
                                                <p className="text-red-500 text-xs mt-4 font-bold">{errors.blik}</p>}
                                        </div>
                                    </div>
                                ) : (
                                    <div className="flex flex-col justify-center flex-grow px-0 sm:px-4">
                                        <div className="text-center mb-6">
                                            ...
                                            <p className="text-xs text-gray-500">Bezpieczne połączenie
                                                z {selectedBank?.toUpperCase()}</p>
                                        </div>
                                        <div className="space-y-4">
                                            <div>
                                                <input type="text" placeholder="Identyfikator / Login"
                                                       value={bankLogin.login}
                                                       onChange={e => onBankLoginChange({
                                                           ...bankLogin,
                                                           login: e.target.value
                                                       })}
                                                       className={getInputClass(errors.bankLogin)}/>
                                                {errors.bankLogin &&
                                                    <p className="text-red-500 text-xs mt-1">{errors.bankLogin}</p>}
                                            </div>
                                            <div>
                                                ...
                                                {errors.bankPass &&
                                                    <p className="text-red-500 text-xs mt-1">{errors.bankPass}</p>}
                                            </div>
                                        </div>
                                    </div>
                                )}
                ...
                {(paymentMethod === 'gpay' || paymentMethod === 'paypal') && (
                    <div className="flex flex-col items-center justify-center h-full animate-fade-in text-center py-6">
                        ...
                        <p className="text-slate-500 text-sm max-w-xs">Bezpieczna finalizacja transakcji nastąpi na
            ...
            <div className="text-center pb-6">
                <p className="text-[10px] text-gray-400">
                    Klikając przycisk, akceptujesz <a href="#"
                                                      className="underline hover:text-gray-600">Regulamin</a> świadczenia
