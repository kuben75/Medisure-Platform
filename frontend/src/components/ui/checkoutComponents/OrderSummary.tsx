...
export default function OrderSummary({
                                         plan, billingPeriod, durationMonths,
    ...
    return (
        <div
            className="w-full lg:w-5/12 xl:w-4/12 bg-[#0F172A] text-white flex flex-col relative flex-shrink-0 lg:h-full lg:overflow-hidden order-first shadow-2xl z-20">
            <div
                className="absolute top-0 right-0 w-[300px] md:w-[500px] h-[300px] md:h-[500px] bg-indigo-500/10 rounded-full blur-[80px] md:blur-[100px] pointer-events-none -mr-20 -mt-20"></div>
            <div
                className="absolute bottom-0 left-0 w-[250px] md:w-[400px] h-[250px] md:h-[400px] bg-blue-500/10 rounded-full blur-[80px] md:blur-[120px] pointer-events-none -ml-20 -mb-20"></div>
            ...
            <div className="relative z-10 flex-grow lg:overflow-y-auto custom-scrollbar px-5 py-4 md:px-10">
                <div className="mb-6">
                    <div className="mb-4 flex flex-wrap gap-2">
                        <span
                            className="inline-block px-3 py-1 bg-white/10 backdrop-blur-md rounded-full text-[10px] font-bold uppercase tracking-wider text-indigo-300 border border-white/5">
                            Twoje zamówienie
                        </span>
                        {hasSavings && (
                            <span
                                className="inline-block px-3 py-1 bg-green-500/20 backdrop-blur-md rounded-full text-[10px] font-bold uppercase tracking-wider text-green-400 border border-green-500/30">
                    ...
                    <div className="space-y-2 mb-6">
                        {featuresList.map((f, i) => (
                            <div key={i} className="flex items-start gap-3 text-sm text-slate-300 group">
                                <div
                                    className="mt-0.5 p-1 rounded-full bg-indigo-500/20 group-hover:bg-indigo-500/30 transition-colors flex-shrink-0">
                    ...
                    {billingPeriod === 'monthly' && durationMonths > 0 && (
                        <div className="mb-4 bg-blue-500/10 border border-blue-500/30 rounded-xl p-3 text-center">
                            ...
                            <p className="text-sm font-medium text-white">
                                Umowa na <span className="font-bold text-blue-300">{durationMonths} miesięcy</span>,
                                płatna w ratach.
                            </p>
                        </div>
                    )}
                    ...
                    <div
                        className="mb-6 bg-white/5 border border-white/10 rounded-xl p-4 flex flex-row items-center justify-between text-sm">
                ...
                <div
                    className="bg-white/5 backdrop-blur-xl rounded-2xl p-4 md:p-5 border border-white/10 shadow-lg mb-6">
                    <div className="space-y-2 mb-5 pb-5 border-b border-white/10">
                        <div className="flex justify-between text-sm">
                            <span className="text-slate-400">Okres</span>
                            <span
                                className="text-slate-200 font-medium">{durationMonths === 0 ? "7 Dni" : `${durationMonths} mies.`}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                    ...
                    <div className="flex flex-col gap-4">
                        {billingPeriod === 'monthly' && (
                            <div
                                className="flex items-center justify-between p-3 rounded-xl bg-indigo-500/10 border border-indigo-500/30 relative overflow-hidden group">
                                <div
                                    className="absolute inset-0 bg-indigo-500/5 group-hover:bg-indigo-500/10 transition-colors"></div>
                        ...
                        <div className="flex justify-between items-end">
                            ...
                            <span className="text-3xl md:text-4xl font-black text-white tracking-tight leading-none">
                                {safeAmountToPay.toFixed(2)} <span
                                className="text-xl text-slate-500 font-bold">zł</span>
                            </span>
                        </div>
                    </div>
                </div>
