...
export default function PackageDetailsModal({isOpen, onClose, plan, userAge, selectedDuration,
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} className="max-w-3xl w-full mx-2 sm:mx-4">
            <div
                className="text-gray-800 relative bg-white rounded-xl shadow-xl max-h-[90vh] overflow-y-auto custom-scrollbar p-4 sm:p-6 md:p-8 mt-4">

                <div
                    className="flex flex-col md:flex-row justify-between items-start gap-4 md:gap-6 mb-6 border-b border-gray-100 pb-6">
                    <div className="flex-grow w-full md:w-auto">
                    <span
                        className="inline-block bg-blue-50 text-[#4E61F6] text-[10px] sm:text-xs font-bold px-2 py-1 rounded mb-2 uppercase tracking-wider">
                        {plan.category}
                    </span>
                    ...
                    <div
                        className="flex items-center gap-3 sm:gap-5 flex-shrink-0 w-full md:w-auto justify-end mt-2 md:mt-0">
                        <div className="text-right">
                            <div className="flex flex-col items-end">
                                ...
                                <div className="flex flex-col items-end mt-1 gap-1">
                                    {!isTestPeriod && (
                                        <span
                                            className="text-[10px] text-gray-500 bg-gray-100 px-2 py-1 rounded whitespace-nowrap">
                                    ...
                                    {billingPeriod === 'upfront' && hasDiscount && (
                                        <span
                                            className="text-[10px] font-bold text-green-700 bg-green-100 px-2 py-0.5 rounded uppercase tracking-wide whitespace-nowrap">
                        ...
                        {user && (
                            <FavoriteButton packageId={plan.id}
                                            className="bg-white border border-gray-200 shadow-sm p-2 sm:p-3 hover:bg-red-50 text-gray-400 hover:text-red-50 transition-colors rounded-full"/>
                        )}
                    </div>
                </div>
                ...
                {!isTestPeriod && (
                    <div className="mb-6 bg-gray-50/80 p-4 sm:p-5 rounded-xl border border-gray-200">
                        ...
                        `}>
                                <div
                                    className="flex items-center gap-3 sm:gap-4 relative z-10 w-full sm:w-auto mb-2 sm:mb-0">
                                    <div
                                        className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors flex-shrink-0 ${billingPeriod === 'monthly' ? 'border-[#4E61F6]' : 'border-gray-300'}`}>
                                        {billingPeriod === 'monthly' &&
                                            <div className="w-2.5 h-2.5 rounded-full bg-[#4E61F6]"/>}
                                    </div>
                                    <div>
                                        <div className="font-bold text-gray-800 text-sm sm:text-base">Płatność
                                            miesięczna
                                        </div>
                                        <div className="text-xs text-gray-500 mt-0.5">Niska rata, większa
                                            elastyczność.
                                        </div>
                                    </div>
                                </div>
                                <div className="pl-8 sm:pl-0 sm:text-right relative z-10">
                                    <span
                                        className="font-bold text-gray-900 text-base sm:text-lg">{displayMonthlyCardPrice} zł</span>
                        ...
                        `}>
                                <div
                                    className="flex items-center gap-3 sm:gap-4 relative z-10 w-full sm:w-auto mb-2 sm:mb-0">
                                    <div
                                        className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors flex-shrink-0 ${billingPeriod === 'upfront' ? 'border-[#4E61F6]' : 'border-gray-300'}`}>
                                        {billingPeriod === 'upfront' &&
                                            <div className="w-2.5 h-2.5 rounded-full bg-[#4E61F6]"/>}
                                    </div>
                                    <div>
                                        <div className="flex flex-wrap items-center gap-2">
                                            ...
                                            {hasDiscount && (
                                                <span
                                                    className="text-[10px] bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-bold border border-green-200 whitespace-nowrap">
                                                -{currentDiscount * 100}%
                                            </span>
                                            )}
                                        </div>
                                        <div className="text-xs text-gray-500 mt-0.5">Jedna płatność. Najniższa cena
                                            całkowita.
                                        </div>
                                    </div>
                                </div>
                                <div className="pl-8 sm:pl-0 sm:text-right relative z-10">
                                    <span
                                        className="font-bold text-gray-900 text-base sm:text-lg">{displayUpfrontCardPrice} zł</span>
                                    <span
                                        className="text-xs text-gray-500 inline sm:block ml-1 sm:ml-0">jednorazowo</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div
                    className="flex items-start gap-3 bg-green-50 p-3 sm:p-4 rounded-xl mb-8 text-sm text-green-800 border border-green-100">
                    <div className="min-w-[20px] pt-0.5 text-xl"><ShieldCheckIcon className={"w-5 h-5"}/></div>
                    <div>
                        <strong className="block mb-1 text-green-900 text-sm">Pełna transparentność i
                            bezpieczeństwo</strong>
                    ...
                    <div className="md:col-span-1 space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {plan.hasDentalCare && <div
                                className="text-xs bg-gray-100 px-2 sm:px-3 py-1 sm:py-2 rounded text-gray-600 font-medium border border-gray-200">🦷
                                Stomatologia</div>}
