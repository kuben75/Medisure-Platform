...
export const PackageFormModal = ({ isOpen, onClose, onPackageAdded, token, packageToEdit }: IPackageFormModalProps) => {
    ...
    const { formData, handleChange, handlePriceValueChange, handleCategoryToggle, handleSubmit, isLoading, error } = usePackageForm(isOpen, packageToEdit, token, () => { onPackageAdded(); onClose(); });
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} className="max-w-6xl w-full">
            <div className="mb-6 border-b border-gray-100 pb-5 flex justify-between items-start">
                ...
                <div className="bg-blue-50 text-[#4E61F6] px-4 py-1.5 mt-4 rounded-full text-xs font-bold uppercase tracking-wider">
            ...
            <form onSubmit={handleSubmit} className="custom-scrollbar pr-2 max-h-[75vh] overflow-y-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 mb-8">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="bg-gray-50/50 p-6 rounded-2xl border border-gray-100">
                            <h4 className="text-sm font-bold text-gray-900 mb-4 flex items-center gap-2">
                                <span className="w-6 h-6 rounded bg-[#4E61F6] text-white flex items-center justify-center text-xs">1</span>
                                Informacje Główne
                            </h4>
                            ...
                            <div className="space-y-5">
                                <div>
                                    ...
                                    <input name="name" type="text" value={formData.name} onChange={handleChange} required className={inputClass} placeholder="np. Pakiet Gold" />
                                </div>
                                <div className="grid grid-cols-2 gap-5">
                                    ...
                                    <div>
                                        ...
                                        <div className="relative">
                                            <input name="priceValue" type="number" step="0.01" value={formData.priceValue} onChange={handlePriceValueChange} required className={`${inputClass} pl-4 pr-12 text-[#4E61F6] font-bold`} />
                                ...
                                <div>
                                    ...
                                    <textarea name="description" value={formData.description} onChange={handleChange} className={inputClass} rows={3} placeholder="Marketingowy opis pakietu..." />
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                />
                                <div className="absolute bottom-3 right-3 text-[10px] text-gray-400 bg-white px-2 py-1 rounded-md border border-gray-100 shadow-sm">
                        ...
                        <div className="grid grid-cols-2 gap-5">
                            ...
                            <div className="opacity-70">
                                ...
                                <input value={formData.specialistsCount} readOnly className={`${inputClass} bg-gray-100 cursor-not-allowed`} />
                            </div>
                        </div>
                    ...
                    <div className="space-y-8 flex flex-col h-full">
                        ...
                        <div className="flex-grow flex flex-col">
                            <h4 className="text-sm font-bold text-gray-900 mb-4 flex items-center gap-2">
                                <span className="w-6 h-6 rounded bg-[#4E61F6] text-white flex items-center justify-center text-xs">2</span>
                                Zarządzanie Zakresem
                            </h4>

                            <div className="bg-white border-2 border-gray-100 rounded-2xl p-1 flex-grow flex flex-col shadow-sm">
                                <div className="p-4 border-b border-gray-100 bg-gray-50 rounded-t-xl flex justify-between items-center">
                                    ...
                                    <span className="text-xs font-bold text-[#4E61F6] bg-blue-100 px-2 py-1 rounded">Wybrano: {currentSpecs.length}</span>
                                </div>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <ModernCheckbox label="Stomatologia" desc="Leczenie zębów" name="hasDentalCare" checked={formData.hasDentalCare} onChange={handleChange}  />
                                <ModernCheckbox label="Szpital" desc="Hospitalizacja" name="hasHospitalization" checked={formData.hasHospitalization} onChange={handleChange}  />
                                <ModernCheckbox label="Rehabilitacja" desc="Fizjoterapia" name="hasRehabilitation" checked={formData.hasRehabilitation} onChange={handleChange}  />
                                <ModernCheckbox label="Wyróżniony" desc="Promuj pakiet" name="isFeatured" checked={formData.isFeatured} onChange={handleChange} variant="yellow"  />
                            </div>
                        </div>
                ...
                <div className="pt-6 border-t border-gray-100 flex justify-end gap-4 bg-white sticky bottom-0 z-10">
                    ...
                    <Button type="submit" variant="primary" className="px-10 py-3 text-sm shadow-xl shadow-blue-600/20" disabled={isLoading}>
...
const ModernCheckbox = ({ label, desc, name, checked, onChange, variant = 'blue', icon }: any) => {
    ...
    return (
        <label className={`relative flex items-center gap-4 p-1 rounded-xl border-2 cursor-pointer transition-all duration-200 select-none ${checked ? activeClass : inactiveClass}`}>
            <input name={name} type="checkbox" checked={checked} onChange={onChange} className="hidden" />

            <div className={`w-5 h-5 rounded-lg flex items-center justify-center transition-colors flex-shrink-0 text-lg shadow-sm border ${checked ? (isYellow ? "bg-yellow-400 border-yellow-400 text-white" : "bg-[#4E61F6] border-[#4E61F6] text-white") : "bg-white border-gray-200 text-gray-400"}`}>
                {checked ? <CheckIcon /> : <span>{icon}</span>}
            </div>

            <div className="flex flex-col">
                <span className={`text-sm font-bold ${checked ? (isYellow ? "text-yellow-900" : "text-[#4E61F6]") : "text-gray-700"}`}>
                    {label}
                </span>
