...
export default function CheckoutOverlay({
    ...
    if (billingPeriod === 'monthly') {
        ...
        if (durationMonths === 0) {
            ...
            effectiveEnd.setMonth(effectiveEnd.getMonth() + durationMonths);
        }
    }
    ...
    const validateStep1 = () => {
        ...
        if (!formData.firstName.trim()) {
            newErrors.firstName = "Imię wymagane";
        }
        if (!formData.lastName.trim()) {
            newErrors.lastName = "Nazwisko wymagane";
        }
        if (!formData.email.trim() || !/\S+@\S+\.\S+/.test(formData.email)) {
            newErrors.email = "Poprawny email wymagany";
        }

        if (!cleanPhone || cleanPhone.length < 9) {
            newErrors.phone = "Wymagany poprawny nr telefonu";
        }
        if (!formData.birthDate) {
            newErrors.birthDate = "Data urodzenia jest wymagana do kalkulacji";
        }

        if (formData.birthDate) {
            const age = calculateAge(formData.birthDate);
            if (age < 18) {
                newErrors.birthDate = "Musisz być pełnoletni";
            }
        }
        ...
        if (!formData.houseNumber.trim()) {
            newErrors.houseNumber = "Nr domu jest wymagany";
        }
        if (!formData.city.trim()) {
            newErrors.city = "Miasto jest wymagane";
        }
        if (!/^\d{2}-\d{3}$/.test(formData.zipCode)) {
            newErrors.zipCode = "Format: XX-XXX";
        }
    ...
    const validateStep2 = () => {
        ...
        if (paymentMethod === 'transfer') {
            ...
            if (selectedBank === 'blik') {
                if (!/^\d{6}$/.test(blikCode)) {
                    newErrors.blik = "Kod BLIK musi mieć 6 cyfr";
                }
            }
            else {
        ...
        if (paymentMethod === 'card') {
            ...
            if (!/^\d{16}$/.test(cleanCard)) {
                newErrors.cardNumber = "Niepoprawny numer karty";
            }
    ...
    return (
        <div
            className="fixed inset-0 z-50 flex flex-col lg:flex-row bg-white animate-fade-in font-sans overflow-y-auto lg:overflow-hidden">
            ...
            <div
                className="w-full lg:w-7/12 xl:w-8/12 bg-white flex flex-col relative order-last lg:h-full lg:overflow-y-auto custom-scrollbar">

                <div
                    className="sticky top-0 bg-white/95 backdrop-blur-md border-b border-gray-100 px-5 py-4 md:px-10 md:py-6 z-30 flex items-center justify-between">
                    <span
                        className="font-bold text-slate-900 text-base md:text-lg tracking-tight">Finalizacja zakupu</span>
                    <div className="flex items-center gap-4 md:gap-6">
                        <div
                            className={`flex items-center gap-2.5 transition-opacity duration-300 ${step === 1 ? 'opacity-100' : 'opacity-40 grayscale'}`}>
                            <div
                                className={`w-6 h-6 md:w-7 md:h-7 rounded-full flex items-center justify-center text-[10px] md:text-xs font-bold shadow-sm transition-colors ${step === 1 ? 'bg-[#4E61F6] text-white' : 'bg-gray-100 text-gray-500'}`}>1
                            </div>
                            <span
                                className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-slate-600 hidden sm:inline-block">Dane</span>
                        </div>
                        ...
                        <div
                            ...
                            <div
                                className={`w-6 h-6 md:w-7 md:h-7 rounded-full flex items-center justify-center text-[10px] md:text-xs font-bold shadow-sm transition-colors ${step === 2 ? 'bg-[#4E61F6] text-white' : 'bg-gray-100 text-gray-500'}`}>2
                            </div>
                            <span
                                className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-slate-600 hidden sm:inline-block">Płatność</span>
                        </div>
                    </div>
                </div>

                <div
                    className="p-5 md:p-10 lg:p-16 max-w-2xl mx-auto w-full flex-grow flex flex-col justify-start pt-6 md:pt-12">
                    {isSuccess ? (
