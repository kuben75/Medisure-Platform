...
export default function NotificationsPanel() {
    ...
    return (
        <div
            className="mt-4 md:mt-6 bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden flex flex-col md:flex-row min-h-[20vh] md:h-[600px] font-sans">

            <div
                className={`w-full md:w-1/3 border-r border-gray-200 bg-slate-50 flex-col ${selectedId ? 'hidden md:flex' : 'flex'}`}>

                <div className="p-4 border-b border-gray-200 bg-white space-y-3">
                    <div className="flex justify-between items-center">
                        <h3 className="font-bold text-gray-800">Skrzynka odbiorcza</h3>
                        <button onClick={markAllAsRead}
                                className="text-xs text-[#4E61F6] hover:underline flex items-center gap-1 font-medium">
                    ...
                    <div className="flex gap-1 p-1 bg-gray-100 rounded-lg">
                        <button onClick={() => setFilter('all')}
                                className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${filter === 'all' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Wszystkie
                        </button>
                        <button onClick={() => setFilter('unread')}
                                className={`flex-1 py-1.5 text-xs font-bold rounded-md transition-all ${filter === 'unread' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Nieprzeczytane
                        </button>
                    </div>
                </div>
                        ...
                        filteredNotifications.map(n => (
                            ...
                            >
                                <div className="flex gap-3">
                                    <div
                                        className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 border ${getIconBg(n.type)}`}>
                                        {getIcon(n.type)}
                                    </div>
                                ...
                                {!n.isRead && <div
                                    className="absolute top-4 right-2 w-2 h-2 bg-[#4E61F6] rounded-full border border-white shadow-sm"></div>}
            ...
            <div className={`w-full md:w-2/3 flex-col bg-white ${selectedId ? 'flex' : 'hidden md:flex'}`}>
                {selectedNotification ? (
                    <div className="flex flex-col h-full animate-fade-in">
                        <div
                            className="p-6 md:p-8 border-b border-gray-100 flex flex-col md:flex-row justify-between items-start bg-slate-50/30 relative">
                            ...
                            <div className="flex items-start gap-4 mt-8 md:mt-0">
                                <div
                                    className={`w-12 h-12 rounded-full flex items-center justify-center shadow-sm border ${getIconBg(selectedNotification.type)}`}>
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-3 mt-2">
                                        <p className="text-xs text-gray-500 flex items-center gap-1">
                                            <CalendarIcon
                                                className="w-4 h-4"/> {new Date(selectedNotification.createdAt).toLocaleString()}
                                        </p>
                                        <span
                                            className="px-2 py-0.5 bg-gray-100 text-gray-500 text-[10px] rounded font-bold uppercase border border-gray-200">
                    ...
                    <div
                        className="flex flex-col items-center justify-center h-full text-gray-400 bg-slate-50/50 p-4 text-center">
                        <div
                            className="w-20 h-20 md:w-24 md:h-24 bg-white rounded-full flex items-center justify-center mb-6 shadow-sm border border-gray-100">
