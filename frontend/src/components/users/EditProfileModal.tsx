        ...
        <Modal isOpen={isOpen} onClose={onClose} className="max-w-xl">
            {step === 'form' ? (
                                    ...
                                    {isBirthDateLocked && (
                                        <div className="group relative">
                                            ...
                                            <div
                                                ...
                                                Data urodzenia została zatwierdzona i wpływa na wycenę pakietów.
                                                <br/><br/>
                                                <span className="text-gray-300">W celu korekty skontaktuj się z Działem Obsługi Klienta.</span>
                                                <div
                ...
                <div className="animate-fade-in text-center px-4">
                    <div
                        className="w-20 h-20 bg-amber-50 rounded-full flex items-center justify-center mx-auto mb-4 text-amber-500 animate-pulse">
                        <ShieldIcon/>
                    </div>
                    <h3 className="text-xl font-bold text-gray-900 mb-2">Potwierdź tożsamość</h3>
                    <p className="text-gray-500 text-sm mb-6">
                        Zmieniasz wrażliwe dane (adres e-mail). <br/>Dla bezpieczeństwa wymagamy autoryzacji.
                    </p>

                    {error && <div
                        className="bg-red-50 text-red-600 p-3 rounded-lg text-sm mb-6 border border-red-100">{error}</div>}
                    ...
                    <div className="space-y-4 text-left max-w-sm mx-auto">
                        ...
                        {user?.twoFactorEnabled && (
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1 ml-1">Kod 2FA
                                    (Authenticator)</label>
                                <input
                                    type="text"
                                    value={twoFactorCode}
                                    onChange={e => setTwoFactorCode(e.target.value.replace(/\D/g, '').slice(0, 6))}
                                    className="w-full px-4 py-3 bg-white border border-gray-300 rounded-xl focus:ring-2 focus:ring-amber-500 outline-none text-center tracking-widest text-lg font-mono"
                                    placeholder="000 000"
                                />
                            </div>
                        )}
                    ...
                    <div className="pt-8 flex gap-3">
                        ...
                        <Button onClick={performUpdate} variant="primary" className="w-2/3 shadow-lg bg-amber-600 hover:bg-amber-700 border-amber-600" disabled={isLoading}>
                            {isLoading ? "Weryfikacja..." : "Potwierdź zmianę"}
                        </Button>
                    </div>
                </div>
            )}
