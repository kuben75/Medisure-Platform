...
export default function ForgotPasswordPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL || "https://localhost:44333/api"}/auth/forgot-password`, {
                method: 'POST',
                        ...
                        <div className="animate-fade-in-up">
                            <div className="bg-green-50 border border-green-100 text-green-800 p-6 rounded-2xl mb-8">
                                ...
                                <p className="text-sm leading-relaxed">
                                    Jeśli konto powiązane z adresem <strong>{maskedEmail(email)}</strong> istnieje, wysłaliśmy na nie link do zmiany hasła.
                                </p>
                        ...
                        <>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-5">
                                <div className="text-left">
                                    ...
                                    <input
                                        id="email"
                                        type="email" required
                                        value={email} onChange={e => setEmail(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#4E61F6] focus:border-transparent outline-none transition-all" disabled={status === 'loading'}/>
                                </div>

                                <Button type="submit" variant="primary" className="w-full py-3.5 text-lg shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all" disabled={status === 'loading'}>
                            ...
                            <div className="mt-8 pt-6 border-t border-gray-100">
                                <p className="text-sm text-gray-500">
                                    Pamiętasz hasło? <Link to="/login" className="font-bold text-[#4E61F6] hover:underline">Zaloguj się</Link>
                                </p>
                            </div>
                        </>
                    )}
