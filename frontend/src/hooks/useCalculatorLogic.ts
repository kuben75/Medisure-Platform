...
export const useCalculatorLogic = () => {
    ...
    const calculateSubscription = (data: ICalculationData) => {
        ...
        setTimeout(() => {
            ...
            if (data.type === 'Biznesowy') {
                targetCategory = 'Biznesowy';
            }
            else if (data.type.includes('Rodzinny')) {
                targetCategory = 'Rodzinny';
            }
            else if (data.type === 'family_custom') {
                targetCategory = 'family_custom';
            }
            else {
                if (data.age >= 60) {
