...
export default function GuideHero() {
    ...
    return (
        <section className="relative w-full text-white py-20 md:py-32 px-4 bg-gradient-to-br from-[#2563EB] via-[#4F46E5] to-[#4338ca] overflow-hidden">

            <div className="absolute top-[-20%] right-[-10%] w-[600px] h-[600px] bg-purple-500/20 rounded-full blur-3xl opacity-30 pointer-events-none"></div>
            <div className="absolute bottom-[-10%] left-[-10%] w-[500px] h-[500px] bg-white/10 rounded-full blur-3xl opacity-20 pointer-events-none"></div>
                        ...
                        {steps.map((step, index) => (
                            <Fragment key={step.number}>
                                <div className="flex-1 flex flex-col relative group">
                                    <div className="bg-white text-gray-800 p-6 md:p-8 rounded-2xl shadow-xl flex flex-col h-full transition-all duration-300 transform hover:-translate-y-2 hover:shadow-2xl border border-white/10">

                                        <div className="flex items-center justify-between mb-4">
                                            <div className="w-10 h-10 md:w-12 md:h-12 flex items-center justify-center rounded-full bg-[#E4E7FE] text-[#4E61F6] font-black text-xl md:text-2xl shadow-inner">
                                                {step.number}
                                            </div>
                                            <div className="w-2 h-2 rounded-full bg-gray-200 group-hover:bg-[#4E61F6] transition-colors"></div>
                                        </div>
